...
**Sync:** This file, [`CLAUDE.md`](CLAUDE.md), and [`.cursor/rules/planerz-guidelines.mdc`](.cursor/rules/planerz-guidelines.mdc) must always contain identical policy. **Whenever you modify any one of them, update the other two as well.**
...
- **French accents:** In French text, always use proper accents and diacritics (e.g. `é`, `è`, `à`, `ç`, `œ`). Never strip accents.
...
## Deployment and version control (agents)
...
- **Scripts with `--apply`:** Do **not** run any script with the `--apply` option (or any equivalent write/execute flag) on your own initiative — on **any** environment (preview, production, local, etc.). Use dry-run or read-only modes when useful for verification. When a script must be applied, tell the product owner and show the exact command; they run it themselves.
